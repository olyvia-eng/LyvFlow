      ...
      >
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Input label="Equipment Name" value={form.name} onChange={(event) => setField('name', event.target.value)} placeholder="e.g. CAT 305 Excavator" />
            <Input label="Type" value={form.type} onChange={(event) => setField('type', event.target.value)} placeholder="e.g. Excavator" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Select label="Status" value={form.status} onChange={(event) => setField('status', event.target.value as EquipmentStatus)}>
              {statuses.map((status) => <option key={status} value={status}>{status.replace('_', ' ')}</option>)}
            </Select>
            <Select label="Cost Type" value={form.costType} onChange={(event) => setField('costType', event.target.value as EquipmentCostType)}>
              {costTypes.map((costType) => <option key={costType} value={costType}>{costType.charAt(0).toUpperCase() + costType.slice(1)}</option>)}
            </Select>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Input label="Hourly Cost" type="number" min={0} value={form.hourlyCost} onChange={(event) => setField('hourlyCost', Number(event.target.value))} />
